...
const Book = ({ order }) => {
  ...
  return (
    <div className="book_list  text-center py-3">
      ...
      {
          (order?.subCategory === "islamic") &&  <Link to={`bookdetails/${order.id}`} className="btn p-0">
          view details
        </Link>
        }
        {
          (order?.subCategory === "uponnas") &&  <Link to={`novelBookdetails/${order.id}`} className="btn p-0">
          view details
        </Link>
        }
      </div>
