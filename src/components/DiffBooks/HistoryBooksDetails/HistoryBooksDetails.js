...
const HistoryBooksDetails = () => {
    const { id } = useParams();
    const [bookItem, setBookItem] = useState({});
    useEffect(() => {
      const singleBook = historyBook.find(book=> book.id == id);
      console.log(singleBook);
      setBookItem(singleBook);
    }, [id]);
    const [category, setCategory] = useState([]);
  useEffect(() => {
    const Books = historyBook.filter(
         ...
         >
           <div className="row">
             ...
             <div className="col-lg-10">
               <p>{bookItem.writer}</p>
               {bookItem.summary ? <p>{bookItem.author}</p> : false}
             </div>
           </div>
         </div>
       </div>
...
export default HistoryBooksDetails;